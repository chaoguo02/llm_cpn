...
def eaSimple(population, toolbox, nodes, pre_generated_taskflows, num_taskflow,
             ...
             decode1,decode2,strategy,result_log,min_fitness_per_gen=None):
    ...
    fitnesses_list = [
        evaluate_offspring_in_parallel(initial_inds, taskflows, nodes, pset,decode1,decode2, work_processing,result_log)
    ...
    # 保存第0代最优个体调度日志和表达式
    record_best_individual_log(elite_inds[0],pre_generated_taskflows,nodes,pset,0,num_run,decode1,decode2,strategy,result_log)
    ...
    for gen in range(1, ngen + 1):
        ...
        # 保存当前代最优个体调度日志和表达式
        record_best_individual_log(elite_inds[0],pre_generated_taskflows,nodes,pset,gen,num_run,decode1,decode2,strategy,result_log)
